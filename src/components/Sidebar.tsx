...
const Sidebar = () => {
    ...
    const filteredUsers = showOnlineOnly ? users?.filter((data) => onlineUsers.includes(data._id)) :users
    if (isUsersLoading) return <SidebarSkeleton />

    return (
        <aside className={`h-full w-${selectedUser ? '20' : 'full'} lg:w-72 border-r border-base-300 flex flex-col transition-all duration-200`}>
...
<div className="overflow-y-auto w-full py-3">
    {filteredUsers.map((user) => (
        ...
        >
            ...
            {/* Optional: Add timestamp or message count indicator */}
